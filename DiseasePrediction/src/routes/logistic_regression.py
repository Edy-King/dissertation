...
logistic_regression_router = APIRouter(
    prefix="/logistic_regression",
    tags=["LogisticRegression"],
)

@logistic_regression_router.post('/predict')
async def logistic_predict(Body: PredictBody):
    ### check if the model exists
    model_path = './models/logistic_regression_model.joblib'

    ### if no model, create the model
    if not os.path.exists(model_path):
        ModelCreator('datasets/covid_data_2020_2021.csv').create_logistic_regression()
    ...
    return {
        ...
        'message': f'Logistic regression model result.The patient with id {Body.hospital_id} is {prediction[0].item()}'
    }